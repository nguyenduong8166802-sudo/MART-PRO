...
app.post('/checkout', async (req,res)=>{
  const cart = getCart(req);
  const { customer_name, phone, address, note, coupon_code } = req.body;
  ...
  const order = await pool.query(
    ...
    [req.session.user?.id || null, customer_name, phone, address, note || '', summary.total, 'new']
  );
  ...
  try {
    await sendZaloOrderNotification({ ...orderRow, total: summary.total, note: `${note || ''}${coupon_code ? ` | Coupon: ${coupon_code}` : ''}` });
  } catch (e) { console.error('Zalo send failed:', e.message); }

  req.session.cart = [];
  res.render('order-success',{ orderId: orderRow.id, summary });
});
